...
//     // 1
//     serialPort.emit("data", [START_SYSEX, 0x6C]);
//     for (i = 0; i < 20; i++) {
...
//     should.deepEqual(serialPort.lastWrite, [START_SYSEX, 0x6F, 46, 52, 1, END_SYSEX]);

//     board.analogWrite(46, 0);
//     should.deepEqual(serialPort.lastWrite, [START_SYSEX, 0x6F, 46, 0, 0, END_SYSEX]);
...
//     board.i2cConfig(1);
//     should.deepEqual(serialPort.lastWrite, [START_SYSEX, 0x78, 1 & 0xFF, (1 >> 8) & 0xFF, END_SYSEX]);
//     done();
//   });
...
//     var request = [START_SYSEX, 0x76, 0x68, 0 << 3, 1 & 0x7F, (1 >> 7) & 0x7F, 2 & 0x7F, (2 >> 7) & 0x7F, 3 & 0x7F, (3 >> 7) & 0x7F, END_SYSEX];
...
//     should.deepEqual(serialPort.lastWrite, [START_SYSEX, 0x76, 0x68, 1 << 3, 4 & 0x7F, (4 >> 7) & 0x7F, END_SYSEX]);
...
//       done();
//     });
//     should.deepEqual(serialPort.lastWrite, [START_SYSEX, 0x6D, 2, END_SYSEX]);
//     serialPort.emit("data", [START_SYSEX]);
...
//     var message = [START_SYSEX, 0x72, 1, 3, board.STEPPER.DIRECTION.CCW, 10000 & 0x7F, (10000 >> 7) & 0x7F, (10000 >> 14) & 0x7F, 2000 & 0x7F, (2000 >> 7) & 0x7F, 3000 & 0x7F, (3000 >> 7) & 0x7F, 8000 & 0x7F, (8000 >> 7) & 0x7F, END_SYSEX];
...
//     serialPort.emit("data", [START_SYSEX, 0x73, 0x42, 0x01, 0x28, 0x36, 0x3F, 0x0F, 0x52, 0x00, 0x00, 0x00, 0x5D, 0x00, END_SYSEX]);
//   });
...
//     serialPort.emit("data", [START_SYSEX, 0x73, 0x45, 0x01, 0x28, 0x36, 0x3F, 0x0F, 0x52, 0x00, 0x00, 0x00, 0x5D, 0x00, END_SYSEX]);
//   });
...
//     serialPort.emit("data", [START_SYSEX, 0x73, 0x43, 0x01].concat(Encoder7Bit.to7BitArray(dataSentFromBoard)).concat([END_SYSEX]));
//   });
...
//     for (var i = 0; i < 5; i++) {
//       serialPort.emit("data", [
//         START_SYSEX, 0x77, 83, 0, 0, 0, 1, 0, 2, 0, 3, 0, 4, 0, END_SYSEX
//       ]);
//     }
...
//     for (var i = 0; i < 5; i++) {
//       serialPort.emit("data", [
//         START_SYSEX, 0x77, 83, 0, 50, 1, 1, 0, 2, 0, 3, 0, 4, 0, END_SYSEX
//       ]);
//     }
